...
def get_remoteok_jobs(keyword):
  ...
  jobs=[]
  jobs_elems = driver.find_elements(By.XPATH, "//table[@id='jobsboard']/tbody/tr[@data-url]")
  
  for job in jobs_elems:
    company = job.get_attribute("data-company")
    position = job.find_element(By.CLASS_NAME, "company_and_position").find_element(By.TAG_NAME, 'a').text
